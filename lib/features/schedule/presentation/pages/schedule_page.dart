...
class _SchedulePageState extends ConsumerState<SchedulePage> {
        ...
        child: Theme(
          ...
          child: Material(
            ...
            child: RepaintBoundary(
              key: boundaryKey,
              child: _ShareableSchedule(matches: matches, playerMap: playerMap),
            ),
          ),
        ),
      ),
  ...
  Widget build(BuildContext context) {
    ...
    final leaguePlayers = ref.watch(
      leagueDetailPlayersProvider(widget.leagueId),
    );
      ...
      data: (players) {
        ...
        return Scaffold(
          appBar: AppBar(
            ...
            actions: [
              if (!scheduleState.isLoading && !scheduleState.isEmpty) ...[
                IconButton(
                  icon: _exporting
                      ? const SizedBox(
                          width: 18,
          ...
          body: scheduleState.isLoading
              ...
              : _ScheduleList(
                  ...
                  onComplete: (matchNumber, teamAWon, scoreA, scoreB) async {
                    final match = scheduleState.matches.firstWhere(
                  ...
                  onEditResult:
                      (matchNumber, newTeamAWon, scoreA, scoreB) async {
                        final match = scheduleState.matches.firstWhere(
                          (m) => m.matchNumber == matchNumber,
                        );
                        ...
                        if (match.teamAWon != newTeamAWon) {
                          final oldWinners = match.teamAWon == true
                              ? match.teamA
                              : match.teamB;
                          final oldLosers = match.teamAWon == true
                              ? match.teamB
                              : match.teamA;
                          await ref
                              .read(playerRepositoryProvider)
                  ...
                  onRemoveMatch: (matchNumber) async {
                    ...
                    if (match.isCompleted && match.teamAWon != null) {
                      final winners = match.teamAWon == true
                          ? match.teamA
                          : match.teamB;
                      final losers = match.teamAWon == true
                          ? match.teamB
                          : match.teamA;
                      await ref
                          .read(playerRepositoryProvider)
          ...
          floatingActionButton:
              !scheduleState.isEmpty && ref.watch(isAdminProvider)
    ...
    return Column(
      children: [
        ...
        Container(
          ...
          child: Row(
            children: [
              Icon(Icons.sports_tennis_rounded, size: 18, color: cs.primary),
...
class _ScheduleList extends StatelessWidget {
  ...
  final void Function(int matchNumber, bool teamAWon, int scoreA, int scoreB)
  onComplete;
  final void Function(int matchNumber, bool newTeamAWon, int scoreA, int scoreB)
  onEditResult;
  ...
  Widget build(BuildContext context) {
    ...
    final roundNumbers = state.matches.map((m) => m.round).toSet().toList()
      ..sort();
          ...
          child: ListView.builder(
            ...
            itemBuilder: (ctx, roundIdx) {
              ...
              final roundMatches = state.matches
                  ...
                  .toList();
              final allDone = roundMatches.every((m) => m.isCompleted);
              final anyActive = roundMatches.any((m) => m.isInProgress);
                ...
                children: [
                  ...
                  ...roundMatches.map((match) {
                    final isCurrent =
                        match.isInProgress ||
                        (match.isScheduled &&
                            state.currentIndex == state.matches.indexOf(match));
                    return Padding(
                      ...
                      child: _MatchCard(
                        ...
                        onEdit: (won, sA, sB) =>
                            onEditResult(match.matchNumber, won, sA, sB),
                ...
                children: [
                  Expanded(
                    child: ChoiceChip(
                      ...
                      selected: teamAWon,
                      onSelected: (_) => setDialogState(() => teamAWon = true),
                    ),
                  ),
                  const SizedBox(width: 8),
                  Expanded(
                    child: ChoiceChip(
                      ...
                      selected: !teamAWon,
                      onSelected: (_) => setDialogState(() => teamAWon = false),
                    ),
                  ),
                ],
              ),
